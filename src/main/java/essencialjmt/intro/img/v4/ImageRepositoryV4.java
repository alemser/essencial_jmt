...
public class ImageRepositoryV4 {
    ...
    public void loadImages(String... imageNames) {        
        CompletableFuture<?>[] cfutures = Arrays.stream(imageNames)
                .map(name -> CompletableFuture.supplyAsync(() -> imageRepo.loadImage(name)).thenApply(this::countColours).thenAccept(imageViewer::display))
